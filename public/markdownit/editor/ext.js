(function(Editor, markdownit, WebUploader){
        ...
        this.$win = $([
            '<div class="modal hide fade" tabindex="-1" role="dialog" aria-labelledby="editorToolImageTitle" aria-hidden="true">',
                '<div class="modal-header">',
                    ...
                    '<h3 id="editorToolImageTitle">添加網址</h3>',
                '</div>',
                ...
                '<div class="modal-footer">',
                    '<button class="btn btn-primary" role="save">確定</button>',
                '</div>',
            '</div>'
        ].join('')).appendTo($body);
        */
    ...
    var ToolImage = function(){
        ...
        this.$win = $([
            '<div class="modal hide fade" tabindex="-1" role="dialog" aria-labelledby="editorToolImageTitle" aria-hidden="true">',
                '<div class="modal-header">',
                    '<button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>',
                    '<h3 id="editorToolImageTitle">圖片</h3>',
                '</div>',
                '<div class="modal-body">',
                    '<div class="upload-img">',
                        '<div class="button">上傳圖片</div>',
                        '<span class="tip"></span>',
        ...
        this.uploader.on('uploadSuccess', function(file, res){
            if(res.success){
                ...
                self.removeFile();
                self.showError(res.msg || '不好意思，上傳失敗');
            }
        });
        ...
        this.uploader.on('error', function(type){
            ...
            switch(type){
                ...
                case 'F_EXCEED_SIZE':
                    self.showError('文件過大, 不能超過2M');
                    break;
                case 'Q_TYPE_DENIED':
                    self.showError('只能上傳圖片');
                    break;
                default:
                    self.showError('發生未知錯誤');
            }
        });

        this.uploader.on('uploadError', function(){
            self.removeFile();
            self.showError('不好意思，上傳失敗');
        });
    };
    ...
    ToolImage.prototype.showFile = function(file){
        ...
        this.file = file;
        this.$uploadBtn.hide();
        this.$uploadTip.html('正在上傳: ' + file.name).show();
        this.hideError();
    };
    ...
    ToolImage.prototype.showProgress = function(file, percentage){
        this.$uploadTip
            .html('正在上傳: ' + file.name + ' ' + percentage + '%')
            .show();
    };
